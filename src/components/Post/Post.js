...
import { witheLessMore } from "../../hoc/witheLessMore";
  ...
  return (
    <div className="post">
      ...
      <div className="post-content">
        ...
        {postText && <p className="description">
          <span>{name} </span> {postText}
        </p>}
        <p className="post-time">{timeAgo}</p>
        {!!comments.length && (isShow ? (
          comments.map((comment) => (
            <p className="description" key={comment.id}>
              <span>{comment.username}</span>
              {comment.body}
            </p>
          ))
        ) : (
          <h2 style={{ cursor: "pointer" }} onClick={() => setIsShow(true)}>
            Show All Comments
          </h2>
        ))}
      </div>
...
export default witheLessMore(Post);
